...
enum {
    ...
    CUBE_VERTS = 24,
    TRI_COUNT = 12,
    TILE_W = 32,      // Reducido para mejor rendimiento en Pi Zero
    TILE_H = 32
};
...
static void scale_framebuffer_to_window(
    const uint8_t *src, int sw, int sh,
    unsigned int *dst, int dw, int dh,
    int ox, int oy, int side)
{
    if (side <= 0) return;
    s32 step_x = (sw << 16) / side;
    s32 step_y = (sh << 16) / side;

    for (int y = 0; y < side; ++y) {
        int sy = (y * step_y) >> 16;
        const uint8_t *src_row = src + sy * sw;
        unsigned int *dst_row = dst + (oy + y) * dw + ox;

        s32 sx_fix = 0;
        for (int x = 0; x < side; ++x) {
            dst_row[x] = g_palette[src_row[sx_fix >> 16]];
            sx_fix += step_x;
        }
    }
}

static XImage *create_window_image(Display *dpy, Visual *visual, int depth, int w, int h, unsigned int **out_pixels) {
    unsigned int *pixels = (unsigned int*)calloc((size_t)w * h, sizeof(unsigned int));
    if (!pixels) return NULL;
    XImage *img = XCreateImage(dpy, visual, depth, ZPixmap, 0, (char*)pixels, w, h, 32, 0);
...
// Sistema de presentación asíncrono
pthread_mutex_t g_present_mutex = PTHREAD_MUTEX_INITIALIZER;
pthread_cond_t g_present_cond = PTHREAD_COND_INITIALIZER;
uint8_t* g_present_fb = NULL;
int g_win_w = 256, g_win_h = 256;
bool g_new_frame_ready = false;
...
void* present_thread_func(void* arg) {
    ...
    unsigned int* win_pixels = NULL;
    XImage* ximage = create_window_image(ctx->dpy, ctx->visual, ctx->depth, g_win_w, g_win_h, &win_pixels);
    int current_w = g_win_w;
    int current_h = g_win_h;
    ...
    while (true) {
        ...
        if (current_w != g_win_w || current_h != g_win_h) {
            current_w = g_win_w;
            current_h = g_win_h;
            if (ximage) { XDestroyImage(ximage); ximage = NULL; win_pixels = NULL; }
            ximage = create_window_image(ctx->dpy, ctx->visual, ctx->depth, current_w, current_h, &win_pixels);
        }
        ...
        if (win_pixels && ximage) {
            memset(win_pixels, 0, (size_t)current_w * (size_t)current_h * sizeof(unsigned int));
            int side = current_h > current_w ? current_w : current_h;
            scale_framebuffer_to_window(local_fb, W, H, win_pixels, current_w, current_h,
                                       (current_w - side) / 2, (current_h - side) / 2, side);
            XPutImage(ctx->dpy, ctx->win, ctx->gc, ximage, 0, 0, 0, 0, current_w, current_h);
            XFlush(ctx->dpy);
        }
    }
...
int main() {
    ...
    Window win = XCreateWindow(dpy, RootWindow(dpy, screen), 0, 0, g_win_w, g_win_h, 0,
    ...
    ColorPacker packer;
    packer.rmask = visual->red_mask;
    packer.gmask = visual->green_mask;
    packer.bmask = visual->blue_mask;
    packer.rshift = mask_shift(packer.rmask);
    packer.gshift = mask_shift(packer.gmask);
    packer.bshift = mask_shift(packer.bmask);
    packer.rbits = count_bits(packer.rmask);
    packer.gbits = count_bits(packer.gmask);
    packer.bbits = count_bits(packer.bmask);

    make_texture_8(texture_8, packer);

    PresentContext pctx;
    pctx.dpy = dpy; pctx.win = win; pctx.gc = gc;
    pctx.visual = visual; pctx.depth = depth;
    ...
    while (running) {
        ...
        if ((now - last_time) >= CLOCKS_PER_SEC) {
            char title[128];
            sprintf(title, "FPS: %.2f | Scalar w-buffer",
